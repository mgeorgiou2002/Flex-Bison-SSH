...
exports.FlexBisonCompletionProvider = void 0;
const vscode = __importStar(require("vscode"));
const bisonParser_1 = require("../utils/bisonParser");
class FlexBisonCompletionProvider {
    //Autocomplete
    provideCompletionItems(document, position) {
        const linePrefix = document.lineAt(position).text.substring(0, position.character);
        const text = document.getText();
        const items = [];
        const addItem = (label, kind, detail, documentation, insertText, sortPrefix = '1') => {
        ...
        if (isBison) {
            ...
            const tokens = (0, bisonParser_1.extractBisonTokens)(text);
            const nonTerminals = (0, bisonParser_1.extractBisonNonTerminals)(text);
            for (const token of tokens) {
                addItem(token, vscode.CompletionItemKind.EnumMember, "Bison token", `Token δηλωμένο με %token: \`${token}\``, undefined, '1');
            }
            for (const nonTerminal of nonTerminals) {
                addItem(nonTerminal, vscode.CompletionItemKind.Reference, "Bison non-terminal", `Non-terminal από grammar rule: \`${nonTerminal}\``, undefined, '2');
            }
        }
        // --- Common C helpers ---
...
exports.FlexBisonCompletionProvider = FlexBisonCompletionProvider;
