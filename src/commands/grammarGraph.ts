...
export function registerGrammarGraphCommand(context: vscode.ExtensionContext) {
    const disposable = vscode.commands.registerCommand('flex.showGrammarGraph', async () => {
        ...
        if (!isBison) {
            vscode.window.showErrorMessage('Grammar Graph works only on Bison files(.y).');
            return;
        }
        ...
        if (rules.length === 0) {
            vscode.window.showWarningMessage('No grammar rules found.');
            return;
        }
        ...
        const panel = vscode.window.createWebviewPanel(
            'grammarGraph',
            `Grammar Graph: ${document.fileName.split(/[\\/]/).pop()}`,
...
function parseGrammarRules(document: vscode.TextDocument): GrammarRule[] {
...
function parseTokenDefinitions(document: vscode.TextDocument): TokenDefinition[] {
...
function extractReferencesFromRuleBody(text: string): string[] {
    let cleaned = text;
...
function clickableMainRuleNode(name: string, line?: number): string {
    return `
        <div
...
function clickableRuleNode(name: string, line?: number): string {
    return `
        <div
...
function clickableTokenNode(name: string, line?: number): string {
    return `
        <div
...
function clickableRuleChip(name: string, line?: number): string {
    return `
        <span
...
function clickableTokenChip(name: string, line?: number): string {
    return `
        <span
...
function clickableCardTitle(name: string, line?: number): string {
    return `
        <span
...
function buildRuleTitle(name: string, line?: number): string {
...
function buildTokenTitle(name: string, line?: number): string {
    const safeName = escapeAttribute(name);
    const safeLine = line !== undefined ? String(line + 1) : 'unknown';
