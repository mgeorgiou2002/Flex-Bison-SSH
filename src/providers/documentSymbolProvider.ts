...
export class FlexBisonDocumentSymbolProvider implements vscode.DocumentSymbolProvider {
    provideDocumentSymbols(
        ...
        const declarationsChildren: vscode.DocumentSymbol[] = [];
        const grammarChildren: vscode.DocumentSymbol[] = [];
        const userCodeChildren: vscode.DocumentSymbol[] = [];
        ...
        for (let i = declarationsStart; i <= declarationsEnd && i < document.lineCount; i++) {
            ...
            if (tokenMatch) {
                ...
                for (const part of parts) {
                    ...
                    declarationsChildren.push(
                        new vscode.DocumentSymbol(
                            part,
                            '%token',
                            vscode.SymbolKind.EnumMember,
                            new vscode.Range(i, 0, i, lineText.length),
                            new vscode.Range(i, startChar, i, startChar + part.length)
                        )
                    );
                }
            }
            ...
            if (startMatch) {
                ...
                declarationsChildren.push(
                    new vscode.DocumentSymbol(
                        ...
                        new vscode.Range(i, 0, i, lineText.length),
                        new vscode.Range(i, startChar, i, startChar + symbolName.length)
                    )
                );
            }
            ...
            if (unionMatch) {
                ...
                declarationsChildren.push(
                    new vscode.DocumentSymbol(
                        ...
                        new vscode.Range(i, 0, endLine, endChar),
                        new vscode.Range(i, 0, i, lineText.length)
                    )
                );
            }
        ...
        if (userCodeStart !== -1) {
            for (let i = userCodeStart; i <= userCodeEnd && i < document.lineCount; i++) {
                ...
                const functionMatch = lineText.match(
                    /^\s*(?:int|void|char|float|double|long|short|FILE|static|const|unsigned|signed)[\w\s\*]*\s+([A-Za-z_][A-Za-z0-9_]*)\s*\([^;]*\)\s*\{?/
                );
                ...
                if (functionMatch) {
                    ...
                    userCodeChildren.push(
                        new vscode.DocumentSymbol(
                            functionName,
                            'User C code',
                            vscode.SymbolKind.Method,
                            new vscode.Range(i, 0, i, lineText.length),
                            new vscode.Range(i, startChar, i, startChar + functionName.length)
                        )
                    );
                }
            }
        ...
        // -----------------------------
        if (firstPercentPercentLine !== -1) {
            const declarationsSection = new vscode.DocumentSymbol(
                'Declarations',
                ...
                new vscode.Range(
                    ...
                    0,
                    declarationsEnd,
                    document.lineAt(Math.min(declarationsEnd, document.lineCount - 1)).text.length
                ),
                new vscode.Range(declarationsStart, 0, declarationsStart, 1)
            );
        ...
        if (grammarStart !== -1 && grammarEnd >= grammarStart) {
            const grammarSection = new vscode.DocumentSymbol(
                'Grammar Rules',
                ...
                new vscode.Range(
                    ...
                    0,
                    grammarEnd,
                    document.lineAt(Math.min(grammarEnd, document.lineCount - 1)).text.length
                ),
                new vscode.Range(grammarStart, 0, grammarStart, 1)
            );
        ...
        if (userCodeStart !== -1 && userCodeEnd >= userCodeStart) {
            const userCodeSection = new vscode.DocumentSymbol(
                'User Code',
                'Bison user code section',
                vscode.SymbolKind.Namespace,
                new vscode.Range(
                    userCodeStart,
                    0,
                    userCodeEnd,
                    document.lineAt(Math.min(userCodeEnd, document.lineCount - 1)).text.length
                ),
                new vscode.Range(userCodeStart, 0, userCodeStart, 1)
            );
