...
interface RuleDefinitionInfo {
    name: string;
    range: vscode.Range;
}

export function getBisonSemanticDiagnostics(document: vscode.TextDocument): vscode.Diagnostic[] {
    ...
    const tokenDecls = new Map<string, vscode.Range[]>();
const knownTokens = new Set<string>();
const ruleDecls = new Map<string, vscode.Range[]>();
const ruleDefinitionsInOrder: RuleDefinitionInfo[] = [];
...
for (const [name, ranges] of tokenDecls.entries()) {
    ...
    diagnostics.push(
        new vscode.Diagnostic(
            ranges[0],
            `Token '${name}' is declared but never used`,
            vscode.DiagnosticSeverity.Warning
        )
    );
}
...
const warnedUnusedRules = new Set<string>();
...
for (const ruleDef of ruleDefinitionsInOrder) {
    ...
    diagnostics.push(
        new vscode.Diagnostic(
            ruleDef.range,
            `Rule '${ruleDef.name}' is defined but never used`,
            vscode.DiagnosticSeverity.Warning
        )
    );
}
...
function collectTokenDeclarations(
    line: string,
    lineNum: number,
    tokenDecls: Map<string, vscode.Range[]>,
...
function collectRuleDefinition(
    line: string,
    lineNum: number,
    ruleDecls: Map<string, vscode.Range[]>,
    ruleDefinitionsInOrder: RuleDefinitionInfo[]
) {
